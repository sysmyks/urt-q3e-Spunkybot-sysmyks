...
class PyQuake3(object):
    ...
    def send_packet(self, data):
        ...
        try:
            ...
            logger.error("Error sending RCON packet: %s", err)
    ...
    def command(self, cmd, timeout=1, retries=1):  # Réduire le nombre de tentatives à 1
        """
        send command and receive response
        """
        # Éviter les commandes vides
        if not cmd or cmd.strip() == 'rcon ""':
            return ["", ""]
            
        # Une seule tentative pour éviter les commandes multiples
        ...
        try:
            ...
            pass
        return ["", ""]  # Retourner une réponse vide au lieu de générer une exception

    def rcon(self, cmd):
        """
        send RCON command
        """
        # Ne pas envoyer de commande vide
