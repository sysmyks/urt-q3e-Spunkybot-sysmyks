...
// Description : Loads clients saved position with improved course handling
/////////////////////////////////////////////////////////////////////
static void SV_LoadPos_f(client_t *cl) {
	int             i, cid, angle;
	qboolean        isReady;
	qboolean        isRunning;
	playerState_t   *ps;
	sharedEntity_t  *ent;
	static int      teleportAttempts[MAX_CLIENTS];
	static int      lastTeleportTime[MAX_CLIENTS];
	...
	isReady = cl->isReady;
	isRunning = cl->isRunning;

	// Check if position saving is allowed
	...
	// Check if position has been saved in cl->cm
	if (!cl->cm.savedPosition[0] && !cl->cm.savedPosition[1] && !cl->cm.savedPosition[2]) {
		// If cl->cm has no saved position, use the QVM system
		VM_Call(gvm, 1, GAME_CLIENT_COMMAND, cl - svs.clients);
		
		// After this call, the position should be loaded into the playerState
		// We can copy it to our cl->cm structure for future use
		VectorCopy(ps->origin, cl->cm.savedPosition);
		VectorCopy(ps->viewangles, cl->cm.savedPositionAngle);
		
		// Return because QVM has already done the teleportation
		return;
	}

	// Case 3: The player is in ready mode AND in a run - warning system
	if (isRunning) {
		int currentTime = svs.time;
		
		// Reset attempts if too much time has passed
		if (currentTime - lastTeleportTime[cid] > 2000) {
			teleportAttempts[cid] = 0;
		}

		teleportAttempts[cid]++;
		lastTeleportTime[cid] = currentTime;
		
		// Ask for confirmation (3 quick presses)
		if (teleportAttempts[cid] < 3) {
			SV_SendServerCommand(cl, "print \"^3Warning:^7 Teleportation will cancel your run. Press again to confirm.^7\"");
			return;
		}

		// After 3 presses, disable the run
		cl->isRunning = qfalse;
		
		// Reset currentCourse if this player was in a run
		if (currentCourse.detected && currentCourse.clientNum == cid) {
			Com_Printf("^5Course canceled for %s (manual teleport)\n", cl->name);
			currentCourse.detected = qfalse;
		}
		
		// Send ready command to QVM to disable the run
	...
	// Case 2: The player is in ready mode but not in a run OR
	// Case 3 after confirmation: Teleport directly without going through the QVM
	// OR new case: player is not in ready mode but has a saved position in cl->cm
	
	// Teleport the player directly using data from cl->cm
...
static void SV_SavePos_f(client_t *cl)
{
	...
	// Case 3: Player is in ready mode AND in a run - block saving
	if (isReady && isRunning) {
		SV_SendServerCommand(cl, "print \"^7You cannot save your position during a run\n\"");
		return;
	}
	
	// Case 1: Player is not in ready mode - normal behavior via QVM
	if (!isReady) {
		// Save position in the cm structure
		VectorCopy(ps->origin, cl->cm.savedPosition);
		VectorCopy(ps->viewangles, cl->cm.savedPositionAngle);
		...
		// Let the QVM do its work as well
		VM_Call(gvm, 1, GAME_CLIENT_COMMAND, cl - svs.clients);
	...
	// Case 2: Player is in ready mode but not in a run - direct save without going through QVM
	else {
		// Save position in the cm structure without going through QVM
		VectorCopy(ps->origin, cl->cm.savedPosition);
		VectorCopy(ps->viewangles, cl->cm.savedPositionAngle);
		
		// Inform the player
		SV_SendServerCommand(cl, "print \"^7Your position has been saved\n\"");
	}

	// Log the action in all cases where the save is performed
	Com_Printf("ClientSavePosition: %d - %f - %f - %f\n", cid, 
			   cl->cm.savedPosition[0], cl->cm.savedPosition[1], cl->cm.savedPosition[2]);
}

/////////////////////////////////////////////////////////////////////
// Name        : SV_Noclip_f
// Description : Toggles noclip mode for a client
/////////////////////////////////////////////////////////////////////
static void SV_Noclip_f(client_t *cl) {
	int clientNum;
	playerState_t *ps;
	
	// Check if player is in ready mode
	if (cl->isReady) {
		SV_SendServerCommand(cl, "print \"^7You cannot use noclip in ready mode\n\"");
		return;
	}
	
	// Check if it's Jump mode
	if (sv_gametype->integer != 9) {  // 9 = Jump mode
		SV_SendServerCommand(cl, "print \"^7Noclip is only available in Jump mode\n\"");
		return;
	}
	
	// Get client ID and playerState
	clientNum = cl - svs.clients;
	ps = SV_GameClientNum(clientNum);
	
	// TRICK: Force cheats only for this command
	Cvar_Set("sv_cheats", "1");
	
	// Pass noclip command to QVM
	Cmd_TokenizeString("noclip");
	VM_Call(gvm, 1, GAME_CLIENT_COMMAND, clientNum);
	
	// Restore cheats status
	Cvar_Set("sv_cheats", "0");
	
	// For additional security, check if noclip was activated
	if (ps->pm_type == PM_NOCLIP) {
		Com_Printf("Noclip enabled for %s\n", cl->name);
	} else {
		Com_Printf("Noclip disabled for %s\n", cl->name);
	}
}
...
================
SV_Ready_f
Sets player's "ready" state
================
*/
static void SV_Ready_f(client_t *cl) {
	// Check if player is a spectator
	playerState_t *ps = SV_GameClientNum(cl - svs.clients);
	qboolean isSpectator = (ps->persistant[PERS_TEAM] == 3); // 3 = TEAM_SPECTATOR
	
	// Spectators cannot be ready
	if (isSpectator) {
		SV_SendServerCommand(cl, "print \"^7Spectators cannot use the ready command\n\"");
		// Silent reset of ready state
		cl->isReady = qfalse;
		return;
	}
...
void SV_ServerModInfo_f(client_t* cl) {
    ...
    SV_SendServerCommand(cl, "chat \"^2==============================================================\"");
    SV_SendServerCommand(cl, "chat \"%s^7Credits: ^3Urt-slim\"", sv_tellprefix->string);
