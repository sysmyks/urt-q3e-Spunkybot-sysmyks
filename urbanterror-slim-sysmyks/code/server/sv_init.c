...
//	Cvar_Set( "nextmap", va("map %s", server) );
	...
	//sysmyks
	// Reset the "ready" state of all clients
	for (i = 0; i < sv_maxclients->integer; i++) {
		svs.clients[i].isReady = qfalse;
		svs.clients[i].isRunning = qfalse;
		svs.clients[i].cm.savedPosition[0] = 0;
		svs.clients[i].cm.savedPosition[1] = 0;
		svs.clients[i].cm.savedPosition[2] = 0;
	...
	currentCourse.clientNum = -1;
	currentCourse.playerName[0] = '\0';
