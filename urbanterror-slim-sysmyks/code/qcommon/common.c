...
#define MAX_NAME_LENGTH 32  // Ajuster selon votre besoin
#endif
...
void QDECL Com_Printf( const char *fmt, ... ) {
	...
	// Add our intercept for course messages here
	if (com_sv_running && com_sv_running->integer && svs.clients) {
		// Patterns to detect the beginning of a course
		if (strstr(msg, "started a jump run") || strstr(msg, "ClientJumpRunStarted")) {
			// Extract player name
			char playerName[MAX_NAME_LENGTH] = {0};
			int i;
			
			// For messages like "sysmyks started a jump run (Way: MarioWay)"
			if (sscanf(msg, "%[^:]:", playerName) == 1 || 
				sscanf(msg, "%[^ ]", playerName) == 1 ||
				sscanf(msg, "broadcast: print \"%[^ ] started", playerName) == 1) {
				
				// Search for the corresponding client
				for (i = 0; i < sv_maxclients->integer; i++) {
					client_t *cl = &svs.clients[i];
					if (cl->state >= CS_CONNECTED && 
						(!Q_stricmp(cl->name, playerName) || 
						(strstr(msg, "ClientJumpRunStarted:") && i == atoi(msg + 19)))) {
						
						cl->isRunning = qtrue;
						currentCourse.detected = qtrue;
						currentCourse.startTime = sv.time;
						currentCourse.clientNum = i;
						Q_strncpyz(currentCourse.playerName, cl->name, sizeof(currentCourse.playerName));
						
						Com_Printf("^5Course detected for %s (client %d)\n", cl->name, i);
						break;
					}
				}
			}
		}
		// Patterns to detect the end of a course
		else if (strstr(msg, "run has been stopped") || strstr(msg, "Time performed by") || 
				strstr(msg, "ClientJumpRunCanceled") || strstr(msg, "ClientJumpRunStopped")) {
			...
			if (sscanf(msg, "%[^:]:", playerName) == 1 || 
				sscanf(msg, "%[^ ]", playerName) == 1 ||
				sscanf(msg, "broadcast: print \"%[^']", playerName) == 1 ||
				sscanf(msg, "broadcast: print \"Time performed by %[^:]", playerName) == 1) {
				
				for (i = 0; i < sv_maxclients->integer; i++) {
					client_t *cl = &svs.clients[i];
					if (cl->state >= CS_CONNECTED && 
						(!Q_stricmp(cl->name, playerName) || 
						((strstr(msg, "ClientJumpRunCanceled:") || strstr(msg, "ClientJumpRunStopped:")) && i == atoi(msg + 21)))) {
						
						cl->isRunning = qfalse;
						
						if (currentCourse.detected && currentCourse.clientNum == i) {
							int runtime = sv.time - currentCourse.startTime;
							Com_Printf("^5Course completed for %s (time: %d ms)\n", cl->name, runtime);
...
//	Swap_Init ();
	...
	Cvar_Get( "sv_master2", MASTER2_SERVER_NAME, CVAR_INIT );
	Cvar_Get( "sv_master3", MASTER3_SERVER_NAME, CVAR_INIT );
